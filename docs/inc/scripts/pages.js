...
define(["require", "exports", "./common", "../../libs/quark-panels/scripts/split"], function (require, exports, bizcommon, splitlib) {
    ...
    var MenuCollection = (function () {
        ...
        /**
          * Menu of home.
          */
        MenuCollection.prototype.home = function () {
            var col = [];
            col.name = "Home";
        ...
        /**
          ...
          */
        MenuCollection.prototype.quark = function () {
            var col = [];
    ...
    function setup(id, path) {
        ...
        extenders.forEach(function (ext, i, arr) {
            ext.load(right);
        });
    }
    exports.setup = setup;
    function genNameHTML(str, styleRef) {
        if (!str)
            return "";
        ...
        if (styleRef === true)
            styleRef = "ali-x-code-class";
        return "<span" + (!!styleRef && styleRef !== false ? " class=\"" + styleRef + "\"" : "") + ">" + name + "</span><span>" + nameA + "</span>";
    }
    function lowerFirst(str) {
        return str.substring(0, 1).toLowerCase() + str.substring(1);
    }
    function objHTML(obj, iip, firstI, singleQuote, thisStr) {
        if (iip === void 0) { iip = 0; }
    ...
    function genOptionsHTML(options, iip, singleQuote) {
        if (iip === void 0) { iip = 0; }
        if (singleQuote === void 0) { singleQuote = false; }
        if (iip == null)
            iip = 0;
        ...
        var str;
        if (!!options)
            str = "<span class=\"ali-x-code-keyword\">function</span> () { " + nl + prefix2 + "<span class=\"ali-x-code-keyword\">var</span> c = <span class=\"ali-x-code-keyword\">this</span>;" + nl + prefix2 + "<span class=\"ali-x-code-keyword\">return</span> " + objHTML(options, iip > 0 ? iip + 1 : -1, false, singleQuote, "c") + ";" + nl + prefix + "}";
        else
            str = "{" + nl + prefix2 + "styleRef: <span class=\"ali-x-code-string\">'sample'</span>," + nl + prefix2 + "attr: { title: <span class=\"ali-x-code-string\">'Test'</span> }" + nl + prefix + "}";
        return str;
    }
    function genControlDescHTML(info) {
        if (!info || !info.name)
            return "";
        ...
        if (cNameI === 0)
            return "";
        if (cNameI > 0)
            cName = cName.substring(0, cNameI);
        ...
        if (hasNs)
            str += "<span class=\"ali-x-code-keyword\">namespace</span> <span class=\"ali-x-code-namespace\">" + info.namespace + "</span> {<br /><br />";
        ...
        if (!!info.options) {
            ...
            if (!!info.templates)
                str += "<div class=\"ali-x-code-unknown\">" + info.templates + "</div><br />";
            ...
            if (!!info.tagName)
                str += "<p>" + bizcommon.strings("usagesWebC2") + "</p><div class=\"ali-x-section-code\">&lt;<span class=\"ali-x-code-tagname\">" + info.tagName + "</span>&gt;<br />&lt;/<span class=\"ali-x-code-tagname\">" + info.tagName + "</span>&gt;</div>";
        }
        ...
        if (!info.hideMembers)
            str += "<h2>" + bizcommon.strings("membersInfo") + "</h2><p>" + bizcommon.strings("seeCommentDoc").replace("{0}", "<code data-mine=\"word\">" + (hasNs ? info.namespace + "." : "") + genNameHTML(info.name) + "</code>") + "</p>";
        return str;
    }
    var ControlDescriptionExtension = (function () {
        function ControlDescriptionExtension() {
            /**
              * Extender name.
              */
            this.name = "quark.site.description.control";
        }
        /**
          * Loads after done.
          * @param control  The target control.
          */
        ControlDescriptionExtension.prototype.load = function (control) {
            if (typeof CodeMirror === "undefined")
                return;
            if (!CodeMirror)
                return;
            var proc = function (newValue) {
                if (!newValue)
                    return;
                setTimeout(function () {
                    ...
                    if (!!codeEles)
                        for (var step = 0; step < codeEles.length; step++) {
                            var codeEle = codeEles[step];
                            if (!codeEle || codeEle._isEditor)
                                continue;
                            codeEle._isEditor = true;
                            var codeHtml = AliHub.Common.Text.trim(codeEle.innerHTML);
                            if (!codeHtml)
                                continue;
                            var classInfo = AliHub.Elements.parseAttr(codeEle, "data-code-control");
                            if (!classInfo)
                                continue;
                            var templates = codeEle.getElementsByTagName("div");
                            if (!templates || templates.length === 0) {
                                if (!classInfo.intro)
                                    classInfo.intro = codeHtml;
                            }
                            else {
                                for (var index in templates) {
                                    ...
                                    if (!!templateKey)
                                        switch (templateKey.toString().toLowerCase()) {
                                            case "intro":
                                                if (!classInfo.intro)
                                                    classInfo.intro = template.innerHTML;
                                                break;
                                            case "templates":
        ...
        return ControlDescriptionExtension;
    }());
    exports.ControlDescriptionExtension = ControlDescriptionExtension;
    var CodeExtension = (function () {
        function CodeExtension() {
            /**
              * Extender name.
              */
            this.name = "quark.site.code";
        }
        /**
          * Loads after done.
          * @param control  The target control.
          */
        CodeExtension.prototype.load = function (control) {
            if (typeof CodeMirror === "undefined")
                return;
            if (!CodeMirror)
                return;
                ...
                setTimeout(function () {
                    ...
                    if (!!codeEles)
                        for (var step = 0; step < codeEles.length; step++) {
                            ...
                            codeEle._isEditor = true;
                            var codeHtml = AliHub.Common.Text.trim(codeEle.innerHTML);
                            if (!codeHtml || codeHtml.length < 50 || codeHtml.indexOf("<") < 0)
                                continue;
                            var langM = AliHub.Elements.getAttr(codeEle, "data-mine");
                            if (langM === "none" || langM === "word")
                                continue;
                            codeEle.innerHTML = "";
